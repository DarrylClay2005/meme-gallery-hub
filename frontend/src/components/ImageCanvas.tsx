...
/**
 * Renders an image onto a Canvas to deter simple saving and applies a watermark.
 * Notes: This cannot stop screenshots or determined users.
 */
export default function ImageCanvas({ src, alt, className, watermarkText, mode = 'visible', rounded = false }: Props) {
  ...
  useEffect(() => {
    let cancelled = false
    const img = new Image()
    // Allow CORS if S3 is configured; if not, the canvas may be tainted but still renders fine.
  ...
  function draw(img: HTMLImageElement) {
    ...
    if (mode !== 'none') {
      paintWatermark(ctx, width, height, watermarkText || defaultWatermark(), mode)
    }
  }

  function defaultWatermark() {
    return import.meta.env.VITE_WATERMARK_TEXT || 'Meme Gallery Hub'
  }

  return (
    <div
      ref={wrapRef}
      className={[className || '', rounded ? 'overflow-hidden rounded' : ''].join(' ').trim()}
